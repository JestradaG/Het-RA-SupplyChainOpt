...
class InitSubproblem:
    ...
    def set_subproblem(self,type=1,risk_averse=False,mu=0.5,c_e=1.5,first_stage=list,leadtime=True,linearized1=True,fixedPenalty=True,linearized2=True,out_of_sample=False,subgradient=False):
        ...
        self.m.setObjective(((1/len(self.m._Omega))*(
        ...
        for (i,k,xi),d in self.m._d.items():
            self.m.addConstr((self.m._u[i,k,xi] + gp.quicksum(self.m._z[j,i,k,xi] for j in self.m._upstream[i] if (j,i,k) in self.m._valid_arcs) >= d - gp.quicksum(self.m._x[j,i,k] for j in self.m._upstream[i] if (j,i,k) in self.m._valid_arcs) ),name=f'Demand_penalty_{i}_{k}_{xi}')
    ...
    def modify_RHS(self,first_stage=None):
        ...
        for (i,k,xi) in self.m._d.keys():
            sum_x = 0
            for j in self.m._upstream[i]:
                if (j,i,k) in self.m._valid_arcs:
