...
def cb_benders(model,where):
    if where == GRB.Callback.MIPSOL:
        ...
        model._sub.modify_RHS(first_stage=[x,y])
...
def solve_using_benders_decomposition(data,compare_optimal=False,set_gap=0.01,c_e=1.5):
        ...
        mp.load_data(data)
        mp.set_masterproblem(risk_averse=False)
        ...
        extended.set_extended(risk_averse=False,
        ...
        if compare_optimal == True:
            start = time.time()
            mp.m._leadtime,mp.m._linearized1,mp.m._fixedPenalty,mp.m._linearized2 = True,False,False,True
            ext = InitExtended()
            ext.load_data(data)
            ext.set_extended(risk_averse=False,
                                leadtime=mp.m._leadtime,linearized1=mp.m._linearized1,
                                fixedPenalty=mp.m._fixedPenalty,linearized2=mp.m._linearized2,c_e=c_e)
        ...
        for (i,j,k) in mp.m._valid_arcs:
            ...
            initial_y[i,j,k] = 1
        mp.m._sub.set_subproblem(risk_averse=False,
                            first_stage=[initial_x,initial_y],
                            leadtime=mp.m._leadtime,linearized1=mp.m._linearized1,
                            fixedPenalty=mp.m._fixedPenalty,linearized2=mp.m._linearized2,
                            subgradient=False,c_e=c_e)
        ...
        mp.m._sub.modify_RHS(first_stage=[mp.m._x_sol,mp.m._y_sol])
